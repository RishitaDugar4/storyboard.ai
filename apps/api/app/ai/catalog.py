...
else in the application outside ``ai/adapters/`` may name a provider or
hard-code one of its constraints.

Graduated from ``tools/bakeoff/catalog.py`` (M6). The bake-off proved these
entries against the live APIs -- model ids, request field names, duration
grids and prices -- so this arrives measured rather than guessed. See
``docs/adr/001-bakeoff-results.md`` for what was actually observed and what is
still only a published figure.
"""
